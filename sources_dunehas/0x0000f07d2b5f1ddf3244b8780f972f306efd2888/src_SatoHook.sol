...
// counterparty: buys mint SATO via the forward curve, sells burn SATO via the
...
// before calling swap (see SatoSwapRouter in the test directory).
// =============================================================================
...
import {SatoToken} from "./SatoToken.sol";
import {Curve} from "./lib/Curve.sol";

/// @title SatoHook
/// @notice The Uniswap V4 hook that mints sato exclusively through a bonding curve.
///         Buys mint SATO via the forward curve; sells burn SATO via the inverse curve,
///         priced against accumulated ETH reserves held by this contract.
/// @dev    See implementation note at the top of this file.
contract SatoHook is IHooks {
    // ---- locked parameters -------------------------------------------------
    ...
    /// @notice The sato ERC-20 token.
    SatoToken public immutable SATO_TOKEN;

    /// @notice The native ETH currency (address(0)).
    Currency public immutable ETH_CURRENCY;

    /// @notice The sato currency wrapper.
    Currency public immutable SATO_CURRENCY;
    ...
    ///         This is what the inverse curve uses to price sells. Differs from `SATO_TOKEN.totalSupply()`
    ...
    /// @notice Reverts when the pool key does not match the configured ETH/SATO/POOL_FEE pair.
    error InvalidPool();
    ...
    /// @param satoToken The sato ERC-20 token (this hook will be set as its sole minter).
    /// @param manifesto The 8x32-byte manifesto string, padded with trailing zeros.
    constructor(IPoolManager poolManager, SatoToken satoToken, bytes32[8] memory manifesto) {
        POOL_MANAGER = poolManager;
        SATO_TOKEN = satoToken;
        ETH_CURRENCY = Currency.wrap(address(0));
        SATO_CURRENCY = Currency.wrap(address(satoToken));
    ...
    /// @notice Pool-init guard. The pool must be ETH/SATO with the configured fee and this hook.
    ...
    function beforeInitialize(address, PoolKey calldata key, uint160) external onlyPoolManager returns (bytes4) {
        ...
        if (Currency.unwrap(key.currency1) != address(SATO_TOKEN)) revert InvalidPool();
    ...
    /// @notice Core trading entrypoint. Handles buys (ETH→SATO) and sells (SATO→ETH) via the
        ...
        returns (bytes4 selector, BeforeSwapDelta delta, uint24 lpFeeOverride)
    {
        ...
        if (Currency.unwrap(key.currency0) != address(0) || Currency.unwrap(key.currency1) != address(SATO_TOKEN)) {
            revert InvalidPool();
        }
    ...
    /// @dev Execute a buy: mint SATO via curve to the manager (delivered to the buyer through
        ...
        returns (bytes4, BeforeSwapDelta, uint24)
    {
        ...
        uint256 mintAmount = _applyEntropy(fairSato, swapper, ethIn);

        // Mint SATO to the manager so the swap delta routes it to the buyer via take.
        POOL_MANAGER.sync(SATO_CURRENCY);
        SATO_TOKEN.mint(address(POOL_MANAGER), mintAmount);
        POOL_MANAGER.settle();
        ...
        // BeforeSwapDelta: specified=+ethIn cancels swap; unspecified=-mintAmount routes SATO to buyer.
        BeforeSwapDelta delta = toBeforeSwapDelta(int128(int256(ethIn)), -int128(int256(mintAmount)));
        return (IHooks.beforeSwap.selector, delta, 0);
    }

    /// @dev Execute a sell: take SATO from the manager (router has pre-settled), burn it,
    ...
    function _executeSell(uint256 satoIn, address swapper)
        internal
        returns (bytes4, BeforeSwapDelta, uint24)
    {
        ...
        uint256 actualSupply = SATO_TOKEN.totalSupply();
        uint256 satoFairIn = (satoIn * totalMintedFair) / actualSupply;
        if (satoFairIn > totalMintedFair) satoFairIn = totalMintedFair;

        // Inverse curve: how much ETH does the curve owe for these tokens (pre-fee).
        uint256 ethRaw = Curve.burnFor(totalMintedFair, satoFairIn);
        ...
        // Take SATO from manager (router pre-settled) and burn it.
        ...
        // BeforeSwapDelta: specified=+satoIn cancels swap; unspecified=-ethOut routes ETH to seller.
        BeforeSwapDelta delta = toBeforeSwapDelta(int128(int256(satoIn)), -int128(int256(ethOut)));
        return (IHooks.beforeSwap.selector, delta, 0);
    }

    /// @dev If we're inside the entropy window, scale `fairSato` by a multiplier in [0.9, 1.1]
    ///      derived from blockhash, the swapper, and the input amount.
    function _applyEntropy(uint256 fairSato, address swapper, uint256 ethIn) internal view returns (uint256) {
