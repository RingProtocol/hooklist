...
/// @title  HypeReserve — owner-only reserve cleanup (external lib).
    ...
    function rebalanceHandler(
        ...
        if (sellTok) {
            ...
            (uint256 ethGot, uint256 tokSold) =
                HypeCore.swapTokenForEth(s, pm, token, amt, TickMath.MAX_SQRT_PRICE - 1);
            ...
            uint256 spent = HypeCore.refillLongBands(s, pm, ethGot, true, numBands);
            s.totalBadDebtETH = s.totalBadDebtETH > spent ? s.totalBadDebtETH - spent : 0;
            if (ethGot > spent) HypeCore.deepenBandsETH(s, pm, ethGot - spent, numBands);
            ...
            (uint256 tokGot, uint256 ethSpent) = HypeCore.swapEthForToken(s, pm, amt, TickMath.MIN_SQRT_PRICE + 1);
            ...
            uint256 spent = HypeCore.refillShortBands(s, pm, token, tokGot, true, numBands);
            s.totalBadDebtTOKEN = s.totalBadDebtTOKEN > spent ? s.totalBadDebtTOKEN - spent : 0;
            if (tokGot > spent) HypeCore.deepenBandsTOKEN(s, pm, token, tokGot - spent, numBands);
