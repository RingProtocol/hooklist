...
/// @title  HypeCore — shared internal primitives (inlined into the handler libs).
/// @notice v4-touching helpers + accounting. `internal` so each external
///         handler-lib (HypeLong/Short/Reserve/Lib) inlines its own copy,
...
library HypeCore {
    ...
    // ─── pool id ────────────────────────────────────────────────────────────
    function poolId(HypeTypes.HypeState storage s) internal view returns (PoolId) {
        return PoolIdLibrary.toId(s.poolKey);
    }

    function currentTick(HypeTypes.HypeState storage s, IPoolManager pm) internal view returns (int24 t) {
        (, t,,) = pm.getSlot0(poolId(s));
    }

    function currentSqrtP(HypeTypes.HypeState storage s, IPoolManager pm) internal view returns (uint160 sp) {
        (sp,,,) = pm.getSlot0(poolId(s));
    }

    // ─── TWAP ring read (shared; inlined into HypeLib/Long/Short) ───────────
    function twapTick(HypeTypes.HypeState storage s, uint32 secondsAgo)
        internal view returns (int24 avgTick, bool ok)
    {
        HypeTypes.Observation memory cur = s.obs[s.obsIndex];
        ...
        HypeTypes.Observation memory past;
        bool found;
        for (uint256 i = 1; i < HypeTypes.OBS_BUFFER_SIZE; ++i) {
            uint256 idx = (uint256(s.obsIndex) + HypeTypes.OBS_BUFFER_SIZE - i) % HypeTypes.OBS_BUFFER_SIZE;
            HypeTypes.Observation memory o = s.obs[idx];
    ...
    function priorBlockTick(HypeTypes.HypeState storage s) internal view returns (int24 tick, bool ok) {
        uint32 nowTs = uint32(block.timestamp);
        for (uint256 i = 0; i < HypeTypes.OBS_BUFFER_SIZE; ++i) {
            uint256 idx = (uint256(s.obsIndex) + HypeTypes.OBS_BUFFER_SIZE - i) % HypeTypes.OBS_BUFFER_SIZE;
            HypeTypes.Observation memory o = s.obs[idx];
            if (!o.initialized) break;
    ...
    function isBlockSpike(HypeTypes.HypeState storage s, int24 liveTick) internal view returns (bool) {
    ...
    function swapEthForToken(HypeTypes.HypeState storage s, IPoolManager pm, uint256 ethIn, uint160 sqrtLimit)
        internal returns (uint256 tokensOut, uint256 ethSpent)
    {
        ...
        int128 e = d.amount0();
        int128 t = d.amount1();
        if (e < 0) { ethSpent = uint256(uint128(-e)); pm.settle{value: ethSpent}(); }
    ...
    function swapEthForExactToken(HypeTypes.HypeState storage s, IPoolManager pm, uint256 tokenOut, uint256 ethBudget)
        internal returns (uint256 gotToken, uint256 ethSpent)
    {
        ...
        uint256 curEth = HypeCurve.ethAtSqrtPrice(currentSqrtP(s, pm));
        uint160 lim = HypeCurve.sqrtPriceX96AtEth(curEth + ethBudget);
    ...
    function swapTokenForEth(HypeTypes.HypeState storage s, IPoolManager pm, address token, uint256 tokenIn, uint160 limit)
        internal returns (uint256 ethOut, uint256 tokenSold)
    {
        ...
        if (e > 0) { ethOut = uint256(uint128(e)); pm.take(CurrencyLibrary.ADDRESS_ZERO, address(this), ethOut); }
    }

    // ─── per-block borrow throttle (ETH-equiv, both sides incl. reserve) ────
    function resetBorrowBlock(HypeTypes.HypeState storage s) internal {
    ...
    // ─── reserve-first draw ─────────────────────────────────────────────────
    function drawReserveETH(HypeTypes.HypeState storage s, uint256 need) internal returns (uint256 drawn) {
        drawn = s.reserveETH < need ? s.reserveETH : need;
        if (drawn > 0) s.reserveETH -= drawn;
    }

    function drawReserveTOKEN(HypeTypes.HypeState storage s, uint256 need) internal returns (uint256 drawn) {
    ...
    function borrowLongBands(
        HypeTypes.HypeState storage s, IPoolManager pm,
        int24 refTick, uint256 want, HypeTypes.Cfg memory cfg
    ) internal returns (uint256 totalFreed) {
        uint256 capPerBand = (HypeCurve.TICK_WIDTH_ETH * cfg.longCapBps) / 10_000;
        uint256 remaining = want;
        uint256 slots;
        for (uint256 b = 0; b < cfg.numBands && remaining > DUST_TOL; b++) {
            if (slots >= cfg.maxBorrowBands) break;
            HypeTypes.Band storage band = s.bands[b];
    ...
    function _pullBandETH(HypeTypes.HypeState storage s, IPoolManager pm, HypeTypes.Band storage band, uint256 take)
        private returns (uint256 freed)
    {
        uint128 lRem = HypeCurve.liquidityForEthOnly(band.v4TickLower, band.v4TickUpper, take);
        ...
        pm.take(CurrencyLibrary.ADDRESS_ZERO, address(this), freed);
        band.liquidity   -= lRem;
    ...
    function borrowShortBands(
        ...
        int24 refTick, uint256 wantTok, HypeTypes.Cfg memory cfg
        ...
        for (uint256 b = 0; b < cfg.numBands && remaining > 0; b++) {
            ...
            uint256 capTok = (HypeCurve.loopAllocForBand(b) * cfg.shortCapBps) / 10_000;
    ...
    function _pullBandTOKEN(HypeTypes.HypeState storage s, IPoolManager pm, HypeTypes.Band storage band, uint256 take)
        private returns (uint256 freed)
    {
        uint128 lRem = HypeCurve.liquidityForLoopOnly(band.v4TickLower, band.v4TickUpper, take);
        if (lRem == 0 || lRem > band.liquidity) return 0;
        (BalanceDelta dd,) = pm.modifyLiquidity(
            s.poolKey,
            ModifyLiquidityParams({ tickLower: band.v4TickLower, tickUpper: band.v4TickUpper,
    ...
    function refillLongBands(
        HypeTypes.HypeState storage s, IPoolManager pm, uint256 budget, bool isHeal, uint256 numBands
        ...
        for (uint256 i = numBands; i > 0 && remaining > 0; i--) {
            HypeTypes.Band storage band = s.bands[i - 1];
            ...
            uint128 lAdd = HypeCurve.liquidityForEthOnly(band.v4TickLower, band.v4TickUpper, put);
            if (lAdd == 0) continue;
            (BalanceDelta dd,) = pm.modifyLiquidity(
                s.poolKey,
                ModifyLiquidityParams({ tickLower: band.v4TickLower, tickUpper: band.v4TickUpper,
    ...
    function refillShortBands(
        HypeTypes.HypeState storage s, IPoolManager pm, address token, uint256 budgetTok, bool isHeal, uint256 numBands
        ...
        for (uint256 b = 0; b < numBands && remaining > 0; b++) {
            ...
            uint256 put  = remaining < target ? remaining : target;
            uint128 lAdd = HypeCurve.liquidityForLoopOnly(band.v4TickLower, band.v4TickUpper, put);
            if (lAdd == 0) continue;
            (BalanceDelta dd,) = pm.modifyLiquidity(
                s.poolKey,
                ModifyLiquidityParams({ tickLower: band.v4TickLower, tickUpper: band.v4TickUpper,
    ...
    function deepenBandsETH(HypeTypes.HypeState storage s, IPoolManager pm, uint256 amount, uint256 numBands)
        internal returns (uint256 deposited)
    {
        ...
        for (uint256 i = numBands; i > 0 && amount > 0; i--) {
            ...
            uint128 lAdd = HypeCurve.liquidityForEthOnly(band.v4TickLower, band.v4TickUpper, amount);
            if (lAdd == 0) continue;
            (BalanceDelta dd,) = pm.modifyLiquidity(
                s.poolKey,
                ModifyLiquidityParams({ tickLower: band.v4TickLower, tickUpper: band.v4TickUpper,
    ...
    function deepenBandsTOKEN(HypeTypes.HypeState storage s, IPoolManager pm, address token, uint256 amount, uint256 numBands)
        internal returns (uint256 deposited)
    {
        if (amount == 0) return 0;
        int24 ct = currentTick(s, pm);
        for (uint256 b = 0; b < numBands && amount > 0; b++) {
            ...
            uint128 lAdd = HypeCurve.liquidityForLoopOnly(band.v4TickLower, band.v4TickUpper, amount);
            if (lAdd == 0) continue;
            (BalanceDelta dd,) = pm.modifyLiquidity(
                s.poolKey,
                ModifyLiquidityParams({ tickLower: band.v4TickLower, tickUpper: band.v4TickUpper,
    ...
    function attributeBadDebtLong(HypeTypes.HypeState storage s, uint256 badDebt, uint256 numBands)
    ...
    function attributeBadDebtShort(HypeTypes.HypeState storage s, uint256 badDebtTok, uint256 numBands)
        internal returns (uint256 flagged)
    {
    ...
    // ─── adaptive fee split (ETH only — spot fee handled in hook) ───────────
    function routeFee(
        HypeTypes.HypeState storage s, uint256 amount, HypeTypes.Cfg memory cfg, address staking
    ) internal {
        if (amount == 0) return;
        ...
        if (toStk > 0) {
            (bool ok,) = staking.call{value: toStk}(abi.encodeWithSelector(IStakingNotify.notifyReward.selector));
            if (!ok) s.claimable[0x98Fb2387eb8B5db1811D6789DE8c1e12546d994D] += toStk;
        }
    }

    // ─── auto-heal: deploy insurance toward bad debt (both sides) ───────────
    function autoHeal(HypeTypes.HypeState storage s, IPoolManager pm, address token, uint256 numBands) internal {
    ...
    function removePosition(HypeTypes.HypeState storage s, uint256 id) internal {
    ...
    function recordHistory(
        HypeTypes.HypeState storage s, address user, uint256 id, uint8 lev,
        HypeTypes.Side side, uint256 collateral, uint256 amtIn, uint256 amtOut, HypeTypes.HistoryKind kind
    ) internal {
        s.userHistory[user].push(HypeTypes.ClosedPositionRecord({
