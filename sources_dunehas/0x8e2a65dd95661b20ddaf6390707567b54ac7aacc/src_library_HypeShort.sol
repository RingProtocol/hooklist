...
/// @title  HypeShort — open/close handler for SHORT positions (external lib).
/// @notice Mirror of HypeLong. Borrow TOKEN reserve-first → ahead bands
...
library HypeShort {
    ...
    function openHandler(
        ...
        uint256 borrowTok = HypeCurve.ethValueToToken(borrowValEth, twapSqrtP);

        HypeCore.resetBorrowBlock(s);

        uint256 reserveDrawn = HypeCore.drawReserveTOKEN(s, borrowTok);
        uint256 remaining    = borrowTok > reserveDrawn ? borrowTok - reserveDrawn : 0;
        uint256 bandsFreed   = remaining > 0 ? HypeCore.borrowShortBands(s, pm, token, refTick, remaining, cfg) : 0;
        uint256 totalFreed   = reserveDrawn + bandsFreed;

        if (totalFreed + HypeCore.DUST_TOL < borrowTok) revert InsufficientBorrowCapacity();

        // Sell the borrowed TOKEN → ETH (hook-internal, fee-exempt).
        (uint256 ethProceeds, uint256 tokSold) =
            HypeCore.swapTokenForEth(s, pm, token, totalFreed, TickMath.MAX_SQRT_PRICE - 1);
        if (tokSold + HypeCore.DUST_TOL < totalFreed) revert PartialFill();
    ...
    function closeHandler(
        ...
        (uint256 gotTok, uint256 ethSpent) =
            HypeCore.swapEthForExactToken(s, pm, tokToRepay, heldPortion);
        if (gotTok == 0) revert NothingSold();

        // Return bought TOKEN → bands first, then deepen bands with leftover.
        uint256 toBands = gotTok;
        uint256 spent = HypeCore.refillShortBands(s, pm, token, toBands, false, cfg.numBands);
        if (toBands > spent) HypeCore.deepenBandsTOKEN(s, pm, token, toBands - spent, cfg.numBands);

        // Couldn't buy back full debt (underwater) → provisional TOKEN bad debt.
        uint256 shortfall = tokToRepay > gotTok ? tokToRepay - gotTok : 0;
        if (shortfall > 0) {
            uint256 flagged = HypeCore.attributeBadDebtShort(s, shortfall, cfg.numBands);
        ...
        if (fullClose) {
            HypeCore.recordHistory(s, pOwner, positionId, lev, HypeTypes.Side.SHORT,
                col, ethSpent, uint256(prevOut) + toUser, HypeTypes.HistoryKind.FullClose);
