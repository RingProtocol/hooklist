...
import {
    IArrakisPrivateHook
} from "../interfaces/IArrakisPrivateHook.sol";
...
contract ArrakisPrivateHook is IHooks, IArrakisPrivateHook {
    ...
    mapping(PoolId => FeesData) private feesData;

    constructor(
        address manager_,
        address factory_
    ) {
        if (manager_ == address(0) || factory_ == address(0)) {
            revert AddressZero();
        }
    ...
    function setFees(
        ...
        if (
            address(IArrakisMetaVault(_vault).module())
                != data_.module
        ) {
            revert NotActiveModule();
        }

        if (
            !IArrakisMetaVaultFactory(factory).isPrivateVault(_vault)
        ) {
    ...
    function beforeAddLiquidity(
        ...
        bytes calldata
    ) external virtual onlyModule(poolKey_, sender) returns (bytes4) {
