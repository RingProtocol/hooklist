...
contract Upeg is IUpeg {
    ...
    /// @dev Ensures Upeg index is within bounds.
    modifier onlyUpegIndex(address owner, uint index) {
    ...
    /// @dev Reorders owner's Upeg: upegId goes to newIndex, others shift.
    function ReorderUpeg(
        address owner,
        uint upegId,
        uint newIndex
    ) external override onlyUpegOwner(upegId) onlyUpegIndex(owner, newIndex) {
        uint currentIndex = _ownedUpegsIndexes[owner][upegId];
        if (newIndex == currentIndex) return;

        // take out element
        uint256 id = _ownedUpegs[owner][currentIndex];

        // shift based on direction
        if (newIndex < currentIndex) {
            // shift right: [newIndex .. currentIndex-1] -> +1
            for (uint i = currentIndex; i > newIndex; i--) {
                _ownedUpegs[owner][i] = _ownedUpegs[owner][i - 1];
                _ownedUpegsIndexes[owner][_ownedUpegs[owner][i]] = i;
            }
        } else {
        ...
        _ownedUpegs[owner][newIndex] = id;
        _ownedUpegsIndexes[owner][id] = newIndex;
    }
    ...
    function ReorderUpegPair(
        ...
        uint256 otherId = _ownedUpegs[owner][newIndex];

        // swap in array
        _ownedUpegs[owner][newIndex] = upegId;
        _ownedUpegs[owner][currentIndex] = otherId;

        // update indices
        _ownedUpegsIndexes[owner][upegId] = newIndex;
        _ownedUpegsIndexes[owner][otherId] = currentIndex;
    }
    ...
    /// @notice Removes holder from the holder list.
    /// @param owner Owner address
    function _removeHolder(address owner) internal {
        delete _holderList[_holderListNumbers[owner]];
    ...
    function transferUpeg(
        address to,
        uint upegId
    ) external override onlyUpegOwner(upegId) {
