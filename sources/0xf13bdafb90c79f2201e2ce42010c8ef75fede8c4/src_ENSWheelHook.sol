...
contract ENSWheelHook is BaseHook, ReentrancyGuard {
    ...
    function updateFeeAddressForCollection(address ENSWheel, address destination) external {
        address collection = ENSWheelFactory.ENSWheelToCollection(ENSWheel);
        if (collection == address(0)) revert InvalidCollection();
        if (IERC721WithOwner(collection).owner() != msg.sender) revert NotCollectionOwner();
        feeAddressClaimedByOwner[ENSWheel] = destination;
    }

    function adminUpdateFeeAddress(address ENSWheel, address destination) external {
        if (msg.sender != ENSWheelFactory.owner() && msg.sender != address(ENSWheelFactory)) {
            revert NotENSWheelFactoryOwner();
        }
        feeAddressClaimedByOwner[ENSWheel] = destination;
    }

    /// @notice Distributes fees: 80% to engine, 20% to team
    function _processFees(address collection, uint256 feeAmount) internal {
        ...
        SafeTransferLib.forceSafeTransferETH(
            feeAddressClaimedByOwner[collection] == address(0) ? feeAddress : feeAddressClaimedByOwner[collection],
            teamAmount
        );
    }

    /// @notice Calculate fee based on blocks since deployment
    function calculateFee(address collection, bool isBuying) public view returns (uint128) {
        if (!isBuying) return DEFAULT_FEE;
        if (ENSWheelFactory.deployerBuying()) return 0;
