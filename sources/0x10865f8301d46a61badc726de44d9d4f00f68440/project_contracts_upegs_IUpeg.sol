...
interface IUpeg {
    ...
    /// @dev Reorders a Upeg for an owner. Shifts all items left/right (may be gas-inefficient).
    /// @param owner Owner address
    /// @param upegId Upeg ID to reorder
    /// @param newIndex New index
    function ReorderUpeg(address owner, uint upegId, uint newIndex) external;
    /// @dev Reorders a Upeg for an owner. Gas-optimized swap without shifting.
    /// @param owner Owner address
    /// @param upegId Upeg ID to reorder
    /// @param newIndex New index or swap index
    function ReorderUpegPair(address owner, uint upegId, uint newIndex) external;
