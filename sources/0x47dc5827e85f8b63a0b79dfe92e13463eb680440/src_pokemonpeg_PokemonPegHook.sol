...
// PokemonPegHook — direct fork of BtcPegHook (v9.3 design, recipient bug fixed).
...
interface IPokemonPegMintable {
    function mintFragmentsFor(address to, uint256 qty) external;
}

contract PokemonPegHook is IHooks, IRandomSeedProvider {
    ...
    uint256 internal constant UNIT_PER_PPEG = 1e18;
    ...
    function afterSwap(
        ...
        bool token1IsPPEG = Currency.unwrap(key.currency1) == address(token);
        int128 ppegDelta = token1IsPPEG ? delta.amount1() : delta.amount0();
        if (ppegDelta <= 0) {
            return (IHooks.afterSwap.selector, 0);
        }

        uint256 ppegBought = uint256(uint128(ppegDelta));
        uint256 qty = ppegBought / UNIT_PER_PPEG;
        if (qty == 0) return (IHooks.afterSwap.selector, 0);

        address recipient = _resolveRecipient(sender, hookData);
        IPokemonPegMintable(address(token)).mintFragmentsFor(recipient, qty);
